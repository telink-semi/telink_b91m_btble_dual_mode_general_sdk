...
//	unsigned short pin = get_pin(cmd.data.para0);
	...
	case PA_SETTING_OPT_OUTPUT:
		if(cmd.data.para1)
		{
			gpio_set_high_level(get_pin(cmd.data.para0));
		}
		else
		{
			gpio_set_low_level(get_pin(cmd.data.para0));
		}
		break;
	default:
		break;
...
{
#if TEST_MODE == BQB
	return get_config_data_byte(addr&0xffff);
#else
	unsigned char ret = 0;
...
{
#if TEST_MODE == BQB
	return get_config_data_half_word(addr&0xffff);
#else
...
#else
	gpio_function_en(PA_TX_EN_PIN);
	gpio_input_dis(PA_TX_EN_PIN);
	gpio_output_en(PA_TX_EN_PIN);
	gpio_set_low_level(PA_TX_EN_PIN);
	gpio_function_en(PA_RX_EN_PIN);
	gpio_input_dis(PA_RX_EN_PIN);
	gpio_output_en(PA_RX_EN_PIN);
...
void pa_operation(pa_setting_state_e state)
{
	...
	case PA_SETTING_STATE_TX:
			//to do:To use the PA function, enable related PA control pins
		gpio_set_high_level(PA_TX_EN_PIN);
    	gpio_set_low_level(PA_RX_EN_PIN);
		break;
	case PA_SETTING_STATE_RX:
			//to do:To use the PA function, enable related PA control pins
		gpio_set_low_level(PA_TX_EN_PIN);
    	gpio_set_high_level(PA_RX_EN_PIN);
		break;
#else
