...
const tlkdrv_codec_modinf_t gcTlkDrvIisSlvInf_b91 = {
...
static tlkdrv_iisslv_t sTlkDrviismstCtrl = {0};
...
static int tlkdrv_iisslv_init(uint08 subDev)
{
	tmemset(&sTlkDrviismstCtrl, 0, sizeof(tlkdrv_iisslv_t));
...
static int tlkdrv_iisslv_open(uint08 subDev)
{
	...
	tlkdrv_iisslv_enable(sTlkDrviismstCtrl.bitDepth, sTlkDrviismstCtrl.channel,
...
static int tlkdrv_iisslv_enable(uint08 bitDepth, uint08 channel, uint32 sampleRate, bool enMic, bool enSpk)
{
	...
	gpio_input_en(I2S_BCK_PC3);
	gpio_input_en(I2S_ADC_LR_PC4);
	gpio_input_en(I2S_ADC_DAT_PC5);
	gpio_input_en(I2S_DAC_LR_PC6);
	gpio_input_en(I2S_DAC_DAT_PC7);
//	audio_i2s_init(0, 0, 0);
	audio_i2s_set_pin();
	audio_set_chn_wl(bitDepth);
	audio_mux_config(IO_I2S, ain0_mode, ain1_mode, i2s_aout_mode);
	audio_i2s_config(I2S_I2S_MODE, bitData, I2S_S_CODEC_M, &audio_i2s_invert_config);
