                                    ...
                                    lv_color_t color, lv_opa_t opa)
{
    ...
    uint8_t * buf_px = buf + (buf_w * y * LV_IMG_PX_SIZE_ALPHA_BYTE + x * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT);
	...
	dest_buf8_apha = buf_px + buf_w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x;
    lv_opa_t bg_opa = dest_buf8_apha[0];
	#else
