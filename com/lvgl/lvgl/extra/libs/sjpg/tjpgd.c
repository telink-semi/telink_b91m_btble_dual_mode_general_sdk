...
)
{
	...
	return 0 - (int)JDR_FMT1;	/* Err: code not found (may be collapsed data) */
}
...
)
{
	...
	if ((d & 0xFFD8) != 0xFFD0 || (d & 7) != (rstn & 7)) {
		return JDR_FMT1;	/* Err: expected RSTn marker is not detected (may be collapsed data) */
	}
	...
	if ((marker & 0xFFD8) != 0xFFD0 || (marker & 7) != (rstn & 7)) {
		return JDR_FMT1;	/* Err: expected RSTn marker was not detected (may be collapsed data) */
	}
