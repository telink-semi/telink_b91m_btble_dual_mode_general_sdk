...
static void new_points_alloc(lv_obj_t * obj, lv_chart_series_t * series, uint32_t cnt, lv_coord_t ** a);
...
void lv_chart_set_type(lv_obj_t * obj, lv_chart_type_t type)
{
    ...
    if(chart->type == LV_CHART_TYPE_SCATTER) {
        ...
        _LV_LL_READ_BACK(&chart->series_ll, series) {
            lv_mem_free(series->x_points);
            series->x_points = NULL;
        }
    }
    ...
    if(type == LV_CHART_TYPE_SCATTER) {
        lv_chart_series_t * series;
        _LV_LL_READ_BACK(&chart->series_ll, series) {
            series->x_points = lv_mem_alloc(sizeof(lv_point_t) * chart->point_cnt);
            LV_ASSERT_MALLOC(series->x_points);
            if(series->x_points == NULL) return;
        }
    }
...
void lv_chart_set_point_count(lv_obj_t * obj, uint16_t cnt)
{
    ...
    _LV_LL_READ_BACK(&chart->series_ll, series) {
        if(chart->type == LV_CHART_TYPE_SCATTER) {
            if(!series->x_ext_buf_assigned) new_points_alloc(obj, series, cnt, &series->x_points);
        }
        if(!series->y_ext_buf_assigned) new_points_alloc(obj, series, cnt, &series->y_points);
...
uint16_t lv_chart_get_x_start_point(const lv_obj_t * obj, lv_chart_series_t * series)
{
    LV_UNUSED(obj);
    LV_ASSERT_NULL(series);

    return series->start_point;
}

void lv_chart_get_point_pos_by_id(lv_obj_t * obj, lv_chart_series_t * series, uint16_t id, lv_point_t * p_out)
{
    LV_ASSERT_NULL(obj);
    LV_ASSERT_NULL(series);
    LV_ASSERT_OBJ(obj, MY_CLASS);
    ...
    if(chart->type == LV_CHART_TYPE_LINE) {
        ...
        p_out->x = lv_map(series->x_points[id], chart->xmin[series->x_axis_sec], chart->xmax[series->x_axis_sec], 0, w);
    }
    else if(chart->type == LV_CHART_TYPE_BAR) {
        uint32_t series_cnt = _lv_ll_get_len(&chart->series_ll);
        int32_t series_gap = ((int32_t)lv_obj_get_style_pad_column(obj,
        ...
        lv_coord_t col_w = block_w / series_cnt;

        p_out->x = (int32_t)((int32_t)w * id) / chart->point_cnt;

        lv_chart_series_t * series_i = NULL;
        _LV_LL_READ_BACK(&chart->series_ll, series_i) {
            if(series_i == series) break;
            p_out->x += col_w;
        }

        p_out->x += (col_w - series_gap) / 2;
    }
    ...
    int32_t temp_y = 0;
    temp_y = (int32_t)((int32_t)series->y_points[id] - chart->ymin[series->y_axis_sec]) * h;
    temp_y = temp_y / (chart->ymax[series->y_axis_sec] - chart->ymin[series->y_axis_sec]);
    p_out->y = h - temp_y;
...
lv_chart_series_t * lv_chart_add_series(lv_obj_t * obj, lv_color_t color, lv_chart_axis_t axis)
{
    ...
    lv_chart_t * chart    = (lv_chart_t *)obj;
    lv_chart_series_t * series = _lv_ll_ins_head(&chart->series_ll);
    LV_ASSERT_MALLOC(series);
    if(series == NULL) return NULL;

    lv_coord_t def = LV_CHART_POINT_NONE;

    series->color  = color;
    series->y_points = lv_mem_alloc(sizeof(lv_coord_t) * chart->point_cnt);
    LV_ASSERT_MALLOC(series->y_points);

    if(chart->type == LV_CHART_TYPE_SCATTER) {
        series->x_points = lv_mem_alloc(sizeof(lv_coord_t) * chart->point_cnt);
        LV_ASSERT_MALLOC(series->x_points);
    }
    if(series->y_points == NULL) {
    ...
    series->start_point = 0;
    series->y_ext_buf_assigned = false;
    series->hidden = 0;
    series->x_axis_sec = axis & LV_CHART_AXIS_SECONDARY_X ? 1 : 0;
    series->y_axis_sec = axis & LV_CHART_AXIS_SECONDARY_Y ? 1 : 0;

    uint16_t i;
    lv_coord_t * p_tmp = series->y_points;
...
void lv_chart_set_x_start_point(lv_obj_t * obj, lv_chart_series_t * series, uint16_t id)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    ...
    series->start_point = id;
}

lv_chart_series_t * lv_chart_get_series_next(const lv_obj_t * obj, const lv_chart_series_t * series)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);

    lv_chart_t * chart  = (lv_chart_t *)obj;
    if(series == NULL) return _lv_ll_get_head(&chart->series_ll);
    else return _lv_ll_get_next(&chart->series_ll, series);
}
...
void lv_chart_set_cursor_point(lv_obj_t * chart, lv_chart_cursor_t * cursor, lv_chart_series_t * series, uint16_t point_id)
{
    ...
    cursor->pos_set = 0;
    if(series == NULL) series = lv_chart_get_series_next(chart, NULL);
...
void lv_chart_set_all_value(lv_obj_t * obj, lv_chart_series_t * series, lv_coord_t value)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    ...
    uint16_t i;
    for(i = 0; i < chart->point_cnt; i++) {
        series->y_points[i] = value;
    }
    series->start_point = 0;
    lv_chart_refresh(obj);
}

void lv_chart_set_next_value(lv_obj_t * obj, lv_chart_series_t * series, lv_coord_t value)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    ...
    invalidate_point(obj, series->start_point);
    series->start_point = (series->start_point + 1) % chart->point_cnt;
    invalidate_point(obj, series->start_point);
}

void lv_chart_set_next_value2(lv_obj_t * obj, lv_chart_series_t * series, lv_coord_t x_value, lv_coord_t y_value)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    ...
    series->x_points[series->start_point] = x_value;
    series->y_points[series->start_point] = y_value;
    series->start_point = (series->start_point + 1) % chart->point_cnt;
    invalidate_point(obj, series->start_point);
}

void lv_chart_set_value_by_id(lv_obj_t * obj, lv_chart_series_t * series, uint16_t id, lv_coord_t value)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    lv_chart_t * chart  = (lv_chart_t *)obj;
    ...
    if(id >= chart->point_cnt) return;
    series->y_points[id] = value;
    invalidate_point(obj, id);
}

void lv_chart_set_value_by_id2(lv_obj_t * obj, lv_chart_series_t * series, uint16_t id, lv_coord_t x_value,
                               lv_coord_t y_value)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    ...
    if(id >= chart->point_cnt) return;
    series->x_points[id] = x_value;
    series->y_points[id] = y_value;
    invalidate_point(obj, id);
}

void lv_chart_set_ext_y_array(lv_obj_t * obj, lv_chart_series_t * series, lv_coord_t array[])
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);

    if(!series->y_ext_buf_assigned && series->y_points) lv_mem_free(series->y_points);
    series->y_ext_buf_assigned = true;
    series->y_points = array;
    lv_obj_invalidate(obj);
}

void lv_chart_set_ext_x_array(lv_obj_t * obj, lv_chart_series_t * series, lv_coord_t array[])
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);

    if(!series->x_ext_buf_assigned && series->x_points) lv_mem_free(series->x_points);
    series->x_ext_buf_assigned = true;
    series->x_points = array;
    lv_obj_invalidate(obj);
}

lv_coord_t * lv_chart_get_y_array(const lv_obj_t * obj, lv_chart_series_t * series)
{
    LV_UNUSED(obj);
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    return series->y_points;
}

lv_coord_t * lv_chart_get_x_array(const lv_obj_t * obj, lv_chart_series_t * series)
{
    LV_UNUSED(obj);
    LV_ASSERT_OBJ(obj, MY_CLASS);
    LV_ASSERT_NULL(series);
    return series->x_points;
}
...
static void lv_chart_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    while(chart->series_ll.head) {
        series = _lv_ll_get_head(&chart->series_ll);

        if(!series->y_ext_buf_assigned) lv_mem_free(series->y_points);
...
static void draw_series_line(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    _LV_LL_READ_BACK(&chart->series_ll, series) {
        ...
        line_dsc_default.color = series->color;
        point_dsc_default.bg_color = series->color;

        lv_coord_t start_point = chart->update_mode == LV_CHART_UPDATE_MODE_SHIFT ? series->start_point : 0;
        ...
        int32_t y_tmp = (int32_t)((int32_t)series->y_points[p_prev] - chart->ymin[series->y_axis_sec]) * h;
        y_tmp  = y_tmp / (chart->ymax[series->y_axis_sec] - chart->ymin[series->y_axis_sec]);
        p2.y   = h - y_tmp + y_ofs;
        ...
        part_draw_dsc.sub_part_ptr = series;
        ...
        for(i = 0; i < chart->point_cnt; i++) {
            ...
            y_tmp = (int32_t)((int32_t)series->y_points[p_act] - chart->ymin[series->y_axis_sec]) * h;
            y_tmp = y_tmp / (chart->ymax[series->y_axis_sec] - chart->ymin[series->y_axis_sec]);
            p2.y  = h - y_tmp + y_ofs;
            ...
            if(i != 0) {
                if(crowded_mode) {
                    if(series->y_points[p_prev] != LV_CHART_POINT_NONE && series->y_points[p_act] != LV_CHART_POINT_NONE) {
                    ...
                    part_draw_dsc.p1 = series->y_points[p_prev] != LV_CHART_POINT_NONE ? &p1 : NULL;
                    part_draw_dsc.p2 = series->y_points[p_act] != LV_CHART_POINT_NONE ? &p2 : NULL;
                    part_draw_dsc.draw_area = &point_area;
                    part_draw_dsc.value = series->y_points[p_prev];
                    ...
                    if(point_w && point_h && series->y_points[p_prev] != LV_CHART_POINT_NONE) {
        ...
        if(!crowded_mode && i == chart->point_cnt) {
            ...
            if(series->y_points[p_act] != LV_CHART_POINT_NONE) {
                lv_area_t point_area;
                ...
                part_draw_dsc.p2 = NULL;
                part_draw_dsc.draw_area = &point_area;
                part_draw_dsc.value = series->y_points[p_act];
...
static void draw_series_scatter(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    _LV_LL_READ_BACK(&chart->series_ll, series) {
        ...
        lv_coord_t p_act = start_point;
        lv_coord_t p_prev = start_point;
        if(series->y_points[p_act] != LV_CHART_POINT_CNT_DEF) {
            p2.x = lv_map(series->x_points[p_act], chart->xmin[series->x_axis_sec], chart->xmax[series->x_axis_sec], 0, w);
            p2.x += x_ofs;

            p2.y = lv_map(series->y_points[p_act], chart->ymin[series->y_axis_sec], chart->ymax[series->y_axis_sec], 0, h);
            p2.y = h - p2.y;
            p2.y += y_ofs;
        }
        else {
...
static void draw_series_bar(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    lv_coord_t col_w = (block_w - (series_cnt - 1) * series_gap) / series_cnt;
...
static void draw_cursors(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
    ...
    _LV_LL_READ_BACK(&chart->cursor_ll, cursor) {
        ...
        if(cursor->pos_set) {
            ...
            lv_point_t p;
            lv_chart_get_point_pos_by_id(obj, cursor->series, cursor->point_id, &p);
            cx = p.x;
            cy = p.y;
        }
    ...
    if(series->start_point != 0) {
        ...
        if(cnt >= point_cnt_old) {
            for(i = 0; i < point_cnt_old; i++) {
                new_points[i] =
                    (*a)[(i + series->start_point) % point_cnt_old]; /*Copy old contents to new array*/
            }
