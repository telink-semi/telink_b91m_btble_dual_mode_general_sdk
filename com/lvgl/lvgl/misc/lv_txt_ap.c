...
typedef struct {
    ...
    int8_t char_attributes_form_offset;
...
void _lv_txt_ap_proc(const char * txt, char * txt_out)
{
    ...
    while(i < txt_length) {
        ...
        if(conjunction_to_previuse && conjunction_to_next)
            ...
            ch_fin[j] = ap_chars_map[index_current].char_end_form + ap_chars_map[index_current].char_attributes_form_offset;
        else if(conjunction_to_previuse && !conjunction_to_next)
    ...
    for(uint8_t i = 0; ap_chars_map[i].char_end_form; i++) {
        if(c == (ap_chars_map[i].char_offset + LV_AP_ALPHABET_BASE_CODE))
            return i;
                ...
                || c == (ap_chars_map[i].char_end_form + ap_chars_map[i].char_attributes_form_offset)     //is it a Beginning form
