                       ...
                       int32_t x_end, lv_color_t * cbuf, uint8_t * abuf)
{
    ...
    for(x = 0; x < x_end; x++) {
        ...
        if(xs_int < 0 || xs_int >= src_w || ys_int < 0 || ys_int >= src_h) {
            ...
            src_tmp += (ys_int * src_stride * LV_IMG_PX_SIZE_ALPHA_BYTE) + xs_int * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;

			#if(TL_LV_OPTIMIZE_ENABLE)
			uint8_t * dest_buf8_apha;
			dest_buf8_apha = src_tmp + src_w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - xs_int;
			#endif
            ...
            cbuf[x].full = *((unsigned short *)src_tmp); //can improve 2ms of all 360*360
#elif LV_COLOR_DEPTH == 32
            cbuf[x].full = *((uint32_t *)src_tmp);
#endif
			#if(TL_LV_OPTIMIZE_ENABLE)
            abuf[x] = dest_buf8_apha[0];
            #else
                            ...
                            int32_t x_end, lv_color_t * cbuf, uint8_t * abuf, lv_img_cf_t cf)
{
    ...
    for(x = 0; x < x_end; x++) {
        ...
        const uint8_t * src_tmp = src;
        src_tmp += (ys_int * src_stride * px_size) + xs_int * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;

		uint8_t * dest_buf8_apha;
		dest_buf8_apha = src_tmp + src_stride*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - xs_int;
        ...
        if(xs_int + x_next >= 0 &&
           ...
           ys_int + y_next <= src_h - 1) {
            ...
            const uint8_t * px_hor = src_tmp + x_next * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
            const uint8_t * px_ver = src_tmp + y_next * src_stride * px_size;

			#if(TL_LV_OPTIMIZE_ENABLE)
            const uint8_t * px_base_alpha = dest_buf8_apha;
            const uint8_t * px_hor_alpha = dest_buf8_apha + x_next;
            const uint8_t * px_ver_alpha = dest_buf8_apha + y_next * src_stride * px_size;
			#endif
            ...
            if(has_alpha) {
                ...
                if(cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
                	#if(TL_LV_OPTIMIZE_ENABLE)
                    a_base = px_base_alpha[0];
                    a_ver = px_ver_alpha[0];
                    a_hor = px_hor_alpha[0];
                    #else
