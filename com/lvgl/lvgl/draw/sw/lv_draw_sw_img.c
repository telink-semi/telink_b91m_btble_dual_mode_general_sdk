...
LV_ATTRIBUTE_FAST_MEM static void convert_cb(const lv_area_t * dest_area, const void * src_buf, lv_coord_t src_w, lv_coord_t src_h,
                       lv_coord_t src_stride, const lv_draw_img_dsc_t * draw_dsc, lv_img_cf_t cf, lv_color_t * cbuf, lv_opa_t * abuf)
{
    LV_UNUSED(draw_dsc);
    LV_UNUSED(src_h);
    ...
    if(cf == LV_IMG_CF_TRUE_COLOR || cf == LV_IMG_CF_TRUE_COLOR_CHROMA_KEYED) {
        ...
        src_tmp8 += (src_stride * dest_area->y1 * LV_IMG_PX_SIZE_ALPHA_BYTE) + dest_area->x1 * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;

        lv_coord_t src_new_line_step_px = (src_stride - lv_area_get_width(dest_area));
        lv_coord_t src_new_line_step_byte = src_new_line_step_px * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
...
//                cbuf[x].full = *src_tmp8 + ((*(src_tmp8 + 1)) << 8);
				cbuf[x].full = *((unsigned short *)src_tmp8); //can improve 2ms of all 360*360
#elif LV_COLOR_DEPTH == 32
          	...
          	#else //Modify the Telinl convert tool, release the color and alpha bytes separately, and do it by copy which can save about 7ms         	
          	lv_memcpy((uint8_t *)cbuf, src_tmp8, dest_w*2); //color
          	lv_memcpy(abuf, src_tmp8 + src_stride*2 - dest_area->x1, dest_w); //alpha
          	#endif
