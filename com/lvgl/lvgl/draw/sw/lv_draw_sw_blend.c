...
LV_ATTRIBUTE_FAST_MEM void lv_draw_sw_blend_basic(lv_draw_ctx_t * draw_ctx, const lv_draw_sw_blend_dsc_t * dsc)
{
    ...
    if(disp->driver->set_px_cb == NULL) {
        if(disp->driver->screen_transp == 0) {
            ...
            dest_buf8 += (blend_area.x1 - draw_ctx->buf_area->x1) * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
...
#if (!TL_LV_OPTIMIZE_ENABLE) //By telink
static inline void set_px_argb(uint8_t * buf, lv_color_t color, lv_opa_t opa)
{
    ...
    bg_color.full = *((unsigned short *)buf); 
...
#else
static inline void set_px_argb(uint8_t * buf, lv_color_t color, lv_opa_t opa, lv_opa_t *bgOpa)
{
    lv_color_t bg_color;
    lv_color_t res_color;
    lv_opa_t bg_opa = bgOpa[0];
#if LV_COLOR_DEPTH == 8
    bg_color.full = buf[0];
    lv_color_mix_with_alpha(bg_color, bg_opa, color, opa, &res_color, &buf[1]);
    if(buf[1] <= LV_OPA_MIN) return;
...
#elif LV_COLOR_DEPTH == 16
    bg_color.full = buf[0] + (buf[1] << 8);
    //bg_color.full = *((unsigned short *)buf); //by Harry
    lv_color_mix_with_alpha(bg_color, bg_opa, color, opa, &res_color, bgOpa);
    if(bgOpa[0] <= LV_OPA_MIN) return;
    buf[0] = res_color.full & 0xff;
    buf[1] = res_color.full >> 8;
#elif LV_COLOR_DEPTH == 32
    bg_color = *((lv_color_t *)buf);
                                            ...
                                            lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
	...
	#if (TL_LV_OPTIMIZE_ENABLE)
	uint8_t * dest_buf8_apha;
    dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
    #endif
    ...
    if(mask == NULL) {
        ...
        /*Has opacity*/
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                	#if(TL_LV_OPTIMIZE_ENABLE)
                    set_px_argb(dest_buf8, color, opa, dest_buf8_apha);
                    dest_buf8 += LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
                    dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x - 1;
                    #else
    ...
    /*Masked*/
        ...
        if(opa >= LV_OPA_MAX) {
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                	#if(TL_LV_OPTIMIZE_ENABLE)
                    set_px_argb(dest_buf8, color,  *mask, dest_buf8_apha);
                    mask++;
                    dest_buf8 += LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
                    dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x - 1;
                    #else
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    if(*mask) {
						...
						#if(TL_LV_OPTIMIZE_ENABLE)
                        set_px_argb(dest_buf8, color,  opa_tmp, dest_buf8_apha);
                        #else
...
#elif 1
							unsigned int *pSrc = (unsigned int *)&src_buf[x];
							unsigned int *pDst = (unsigned int *)&dest_buf[x];
							pDst[0] = pSrc[0];
							pDst[1] = pSrc[1]; //can improve 4ms of all 360*360
#endif
            ...
            if(blend_fp == NULL && LV_COLOR_DEPTH == 32) {
                ...
                for(y = 0; y < h; y++) {
                    if(blend_fp == NULL) {
                        for(x = 0; x < w; x++) {
                        	#if( TL_LV_OPTIMIZE_ENABLE )
                            set_px_argb(dest_buf8, src_buf[x], LV_OPA_COVER, dest_buf8_apha);
                            dest_buf8 += LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
							dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x - 1;
							#else
        ...
        /*No mask but opacity*/
            ...
            for(y = 0; y < h; y++) {
                if(blend_fp == NULL) {
                    for(x = 0; x < w; x++) {
                    	#if( TL_LV_OPTIMIZE_ENABLE )
                        set_px_argb(dest_buf8, src_buf[x], opa, dest_buf8_apha);
                        dest_buf8 += LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
						dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x - 1;
						#else
        ...
        if(opa > LV_OPA_MAX) {
            ...
            for(y = 0; y < h; y++) {
                if(blend_fp == NULL) {
                    for(x = 0; x < w; x++) {
                    	#if( TL_LV_OPTIMIZE_ENABLE )
                        set_px_argb(dest_buf8, src_buf[x], mask[x], dest_buf8_apha);
                        dest_buf8 += LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;
						dest_buf8_apha = dest_buf8 + w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x - 1;
						#else
                ...
                if(blend_fp == NULL) {
                    for(x = 0; x < w; x++) {
                        if(mask[x]) {
                            ...
                            #if( TL_LV_OPTIMIZE_ENABLE )
                            set_px_argb(dest_buf8, src_buf[x], opa_tmp, dest_buf8_apha);
                            #else
