...
lv_opa_t lv_img_buf_get_px_alpha(lv_img_dsc_t * dsc, lv_coord_t x, lv_coord_t y)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
        uint32_t px = dsc->header.w * y * LV_IMG_PX_SIZE_ALPHA_BYTE + x * LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT;

		#if( TL_LV_OPTIMIZE_ENABLE )
		uint8_t * dest_buf8_apha = buf_u8 + px + dsc->header.w*LV_IMG_PX_SIZE_ALPHA_BYTE_NEW_FORMAT - x;
        return dest_buf8_apha[0];
        #else
