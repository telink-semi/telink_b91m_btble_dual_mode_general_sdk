...
HCI_Read_Timing_Accuracy_req(handle_w,"uart",handle_cara)
while(1)
do
    result_tbl2,result_len2 = HCI_result_read(handle_r,"uart");
    ret_type,code,name,flag,rst_tbl = dongle_print_process(result_tbl2,result_len2);
	...
	---step 1, check cmd status rsp
        -- if (name == "HCI_Command_Status") then
			-- if(rst_tbl.Command_Opcode == 0x3f01) then
				-- if (rst_tbl.Status == 0) then
					-- print("Read_Timing_Accuracy_req");
				-- end 
			-- end
        -- end

	---step 2, check evt: 
        if (name == "HCI_Read_Timing_Accuracy_res") then
			if(flag) then
                print("LMP_TIMING_ACCURACY_RES");
				table.print(rst_tbl);
				break
			end
		end
