...
function HCI_Wait_Command_Complete(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Command Complete: Timeout",0,0)
			break
		end
	end
end

function HCI_Wait_Command_Status(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Command Status Event: Timeout",0,0)
			break
		end
	end	
end
...
function HCI_Wait_Inquiry_Complete(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		result_tbl,result_len  = HCI_Result_Read(handle_r,mode)
		code,name,flag,rst_tbl = HCI_Event_Exec(result_tbl,result_len,mode);	
		if(name == "HCI_Inquiry_Complete") then
			if(flag) then
				print_table(HCI_EVENT_TABLE_DISPLAY,rst_tbl);
				break;
			end
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Inquiry Complete Event: Timeout",0,0)
			break
		end
	end	
end
...
function HCI_Wait_Connect_Request(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		result_tbl,result_len  = HCI_Result_Read(handle_r,mode)
		-- dongle_print_process(result_tbl,result_len)
		code,name,flag,rst_tbl = HCI_Event_Exec(result_tbl,result_len,mode);	
		if(name == "HCI_Connection_Request") then
			if(flag) then
				print_table(HCI_EVENT_TABLE_DISPLAY,rst_tbl);
				return flag,rst_tbl.BD_ADDR
			end
		end
		if(timeout_sec(t0,10)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Connection Request Event: Timeout",0,0)
			return nil,nil
		end
	end	
end

function HCI_Wait_Connect_Complete(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Connection Complete Event: Timeout",0,0)
			return nil,nil
		end
	end	
end

function HCI_Wait_Disconnection_Complete(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		result_tbl,result_len  = HCI_Result_Read(handle_r,mode)
		code,name,flag,rst_tbl = HCI_Event_Exec(result_tbl,result_len,mode);	
		if(name == "HCI_Disconnection_Complete") then
			if(flag) then
				print_table(HCI_EVENT_TABLE_DISPLAY,rst_tbl);
				return flag,rst_tbl
			end
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Disconnection Complete Event: Timeout",0,0)
			return nil,nil
		end
	end	
end
...
function HCI_Wait_Max_Slot(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		result_tbl,result_len  = HCI_Result_Read(handle_r,mode)
		code,name,flag,rst_tbl = HCI_Event_Exec(result_tbl,result_len,mode);	
		if(name == "HCI_Max_Slots_Change") then
			if(flag) then
				print_table(HCI_EVENT_TABLE_DISPLAY,rst_tbl);
				return flag,rst_tbl
			end
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Max Slots Change Event: Timeout",0,0)
			return nil,nil
		end
	end	
end

function HCI_Wait_Read_Clock_Offset_Complete(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		result_tbl,result_len  = HCI_Result_Read(handle_r,mode)
		code,name,flag,rst_tbl = HCI_Event_Exec(result_tbl,result_len,mode);	
		if(name == "HCI_Read_Clock_Offset_Complete") then
			if(flag) then
				print_table(HCI_EVENT_TABLE_DISPLAY,rst_tbl);
				return flag,rst_tbl
			end
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Read Clock Offset Complete Event: Timeout",0,0)
			return nil,nil
		end
	end
end
...
function HCI_Wait_Role_Change(handle_r,mode)
	t0 = os.time()
	while(1)
	do 
		--read and parse the corresponding HCI Command Complete Event to the HCI_Reset Command
		...
		if(timeout_sec(t0,5)==1) then
			log_printf(HCI_EVENT_TIMEOUT_DISPLAY,"HCI Role Change Event: Timeout",0,0)
			return nil,nil
		end
	end
end
...
function EVENT_HCI_Sniff_Subrating(tbl,mode)
	...
	rst_tbl.Connection_Handle = tbl[4+offset] + bit32.lshift(tbl[5+offset],8);
	rst_tbl.Maximum_Transmit_Latency = tbl[6+offset] + bit32.lshift(tbl[7+offset],8);
	rst_tbl.Maximum_Receive_Latency = tbl[8+offset] + bit32.lshift(tbl[9+offset],8);
	rst_tbl.Minimum_Remote_Timeout = tbl[10+offset] + bit32.lshift(tbl[11+offset],8);
	rst_tbl.Minimum_Local_Timeout = tbl[12+offset] + bit32.lshift(tbl[13+offset],8);
	return true, rst_tbl;
end
