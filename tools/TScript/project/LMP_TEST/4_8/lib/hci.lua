...
function HCI_Enable_Device_Under_Test_Mode (handle,mode)
	print("HCI_Enable_Device_Under_Test_Mode");
	len = 0;
	local a = array.new(32);
	offset = 0;
	if(mode=="uart") then
...
function  HCI_Write_Connection_Accept_Timeout(handle,mode,Connection_Accept_Timeout)
	print(string.format("HCI_Write_Connection_Accept_Timeout: 0x%04x",Connection_Accept_Timeout))
	len = 0;
	offset = 0;
	local a = array.new(32);
	...
	a[1+offset] = 0x16;  --OCF =0x0f
	a[2+offset] = 0x0c;  --OGF =1
	a[3+offset] = 0x02;  --para len
	
	a[4+offset] = bit32.band(Connection_Accept_Timeout, 0xff);
	a[5+offset] = bit32.band(bit32.rshift(Connection_Accept_Timeout,8) , 0xff);
