...
if(flag==true) then
	if(rst_tbl.Maximum_Transmit_Latency~=20 or rst_tbl.Maximum_Receive_Latency~=80 or rst_tbl.Minimum_Remote_Timeout~=0 or rst_tbl.Minimum_Local_Timeout~=320) then
		Test_Status="Fail"
		goto Test_Case_End
	end
else
...
flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Sniff_Subrating_Result",Timeout)
if(flag==true) then
	if(rst_tbl.Maximum_Transmit_Latency~=160 or rst_tbl.Maximum_Receive_Latency~=80 or rst_tbl.Minimum_Remote_Timeout~=160 or rst_tbl.Minimum_Local_Timeout~=640) then
		Test_Status="Fail"
		goto Test_Case_End
	end
else
