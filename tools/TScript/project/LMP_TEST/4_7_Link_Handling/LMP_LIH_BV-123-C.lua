...
-- => LT:HCI_Sniff_Subrating
-- HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Sniff_Subrating_Result",Timeout)
...
flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Sniff_Subrating_Result",Timeout)
if(flag==true) then
	if(rst_tbl.Maximum_Transmit_Latency~=80 or rst_tbl.Maximum_Receive_Latency~=240 or rst_tbl.Minimum_Remote_Timeout~=320 or rst_tbl.Minimum_Local_Timeout~=480) then
		Test_Status="Fail"
		goto Test_Case_End
	end
else
