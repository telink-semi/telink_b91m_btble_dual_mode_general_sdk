...
IUT_ACTING_MODE   = MASTER
--IUT_ACTING_MODE   = SLAVE
Local_name = "IUT0"
...
do 
	...
	if(step == 1) then
		if(name == "HCI_Command_Complete") then
			if(flag) then
				...
				--print("CSR dongle 
					if(IUT_ACTING_MODE ~= MASTER)then 
						step = 20;
	...
	if(step == 20) then
		if(name == "HCI_Command_Complete") then
			if(flag) then
				  if( rst_tbl.Command_Opcode == 0x0c1a) then
					step = 3;
		...
		-- if(name == "HCI_Command_Complete") then
			-- if(flag) then
				  -- if( rst_tbl.Command_Opcode == 0x0c20) then
				    -- --print("send HCI_Write_Local_Name");
					-- --break;
					-- HCI_Write_Local_Name(handle,mode,"IUT0");
					-- step = 4;
					-- --HCI_Write_Authentication_Enable(handle_tb,"usb",);
				  -- end
				-- --print(string.format("step 2: 0x%04x",rst_tbl.Command_Opcode));
			-- end
		-- end
...
--]]
-- step 2:  step 2: Connection Establishment lower Tester
print("******************************************************")
print("  step 2: Connection Establishment lower Tester ---   ")
print("******************************************************")
-- 注意这里的BD_ADDR是个二维数组
addr ={};
addr[1] = 0xe6;
...
do 
	...
	if step == 2 then
		HCI_Read_Clock_Offset(handle_w,"usb",acl_handle); 
		step = 3;
	end
	...
	if(step == 3) then
		if(name2 == "HCI_Command_Status") then
				table.print(rst_tbl2);
				if(flag2) then
					if((rst_tbl2.Status==0) and (rst_tbl2.Command_Opcode == 0x41f)) then
...
son with automatic accept allowed.
	...
	if(step == 8) then
		if(name2 == "HCI_User_Confirmation_Request") then
			if(flag2) then
				  print(string.format("number = %x",rst_tbl2.Numeric_Value));
...
son with automatic accept allowed.
	...
	if(step == 9) then
		...
		if(name == "HCI_Command_Complete") then
			if(flag) then
					table.print(rst_tbl);
				  if( rst_tbl.Command_Opcode == 0x042c) then
					step = 10;
				  end
				--print(string.format("step 5: 0x%04x",rst_tbl.Command_Opcode));
			end
