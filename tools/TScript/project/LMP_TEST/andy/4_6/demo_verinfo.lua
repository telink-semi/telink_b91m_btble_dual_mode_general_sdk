...
---sent a HCI cmd to cara to request version info from csr---
HCI_Read_Remote_Version_Information(handle_w,"uart",handle_cara)
while(1)
do
    result_tbl2,result_len2 = HCI_result_read(handle_r,"uart");
    ret_type,code,name,flag,rst_tbl = dongle_print_process(result_tbl2,result_len2);
	...
	if(rst_tbl.Command_Opcode == 0x041d) then
            if (rst_tbl.Status == 0) then
                print("cara echo cmd ver_req status ok");
                --print(string.format("caracara ACL handle = 0x%04x",rst_tbl.Connection_Handle));
		--handle_cara = rst_tbl.Connection_Handle;
            end 
         end
	...
	---step 2, check evt: 
        if (name == "HCI_Read_Remote_Version_Information_Complete") then
	if(flag) then
            --if (rst_tbl.Status == 0) then --???
                print("the remote rsp ver_info ok");
		table.print(rst_tbl);
                --print(string.format("caracara ACL handle = 0x%04x",rst_tbl.Connection_Handle));
		--handle_cara = rst_tbl.Connection_Handle;
            --end 
