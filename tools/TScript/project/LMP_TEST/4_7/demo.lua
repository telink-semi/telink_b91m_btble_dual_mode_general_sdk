...
print("CSR dongle ----> caracara (simple pairing)");
flag = false;
step = 1;
acl_handle = 0;
while(1)
		...
		if(name2 == "HCI_Connection_Complete") then
			 ...
			 if(flag2) then
				if(rst_tbl2.Status==0) then
				   ...
				   csr_acl_handle = rst_tbl2.Connection_Handle;
...
son with automatic accept allowed.
	...
	if(step == 8) then
		if(name == "HCI_User_Confirmation_Request") then
			if(flag) then
				  print(string.format("number = %d",rst_tbl.Numeric_Value));
				  g_cfm_csr = 1;
		...
		if((g_cfm_caracara==1) and (g_cfm_csr == 1)) then
			print("step : 8--> 9");
			HCI_User_Confirmation_Request_Reply(handle_tb,"usb",addr);
			HCI_User_Confirmation_Request_Reply(handle_w,"uart",csr_addr);
			g_cmd_done_csr = 0;
			g_cmd_done_caracara = 0;
			step = 9;
		end
	...
	if(step == 9) then
		...
		if(g_cmd_done_csr == 1) and (g_cmd_done_caracara == 1) then
			print("step 9---> 10");
			g_cmd_done_csr = 0;
			g_cmd_done_caracara = 0;
			step = 10;
