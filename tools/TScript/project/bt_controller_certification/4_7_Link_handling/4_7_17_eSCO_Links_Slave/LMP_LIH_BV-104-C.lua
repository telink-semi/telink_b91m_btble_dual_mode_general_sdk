...
function LMP_LIH_BV_104_C(iut_mode,lt_mode,role,Timeout)

    Testcase = "LMP/LIH/BV-104-C"
    -- iut_mode = "dongle"
    -- lt_mode  = "usb"
    OPTIONAL = ALT1
    print("--------------------<<   "..Testcase.."   >>--------------------")
...
--  <= LT:wait for HCI_Synchronous_Connection_Complete Event
    ...
    print("============ LT initiates Synchronous Link Parameters Change =============")
    if (lt_sync_con_hdl==nil) then
        Test_Status = "Fail"
        goto Test_Case_End
    end
    Max_Latency           = 18;         --18ms
    ...
    HCI_Setup_Synchronous_Connection(lt_handle_w,lt_mode,lt_sync_con_hdl,Transmit_Bandwidth,Receive_Bandwidth,Max_Latency,Voice_Setting,Retransmission_Effort,Packet_Type);
    flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
    if(flag~=true) then
        Test_Status = "Fail"
        goto Test_Case_End
...
LMP_LIH_BV_104_C("dongle","usb","slave",10)