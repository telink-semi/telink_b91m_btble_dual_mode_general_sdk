...
function LMP_LIH_BV_115_C(iut_mode,lt_mode,role,Timeout)

    Testcase = "LMP/LIH/BV-115-C"
    -- iut_mode = "dongle"
    -- lt_mode  = "usb"
    OPTIONAL = ALT1
    print("--------------------<<   "..Testcase.."   >>--------------------")
...
--  <= LT:wait for HCI_Synchronous_Connection_Complete Event
    ...
    -- trigger LT(csr dongle) to disconnect the eSCO link with IUT(eagle dongle)
    print("============ LT disconnects the eSCO link =============")
    HCI_Disconnect (lt_handle_w,lt_mode,lt_sync_con_hdl,0x13);
    -- => LT:HCI Command Status event  
    flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
    if(flag~=true) then
        Test_Status = "Fail"
...
LMP_LIH_BV_115_C("dongle","usb","master",10)