...
function LMP_LIH_BV_16_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/LIH/BV-16-C"
	-- iut_mode = "uart"
	-- lt_mode  = "usb"
	OPTIONAL = ALT1
	print("--------------------<<   "..Testcase.."   >>--------------------")
...
--[[
	...
	-- <= LT:HCI_Exit_Sniff_Mode
	HCI_Exit_Sniff_Mode(lt_handle_w,lt_mode,lt_Connection_Handle)
	-- => LT:HCI Command Complete event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
	if(flag~=true) then
		goto Test_Case_End
	...
	-- => IUT:HCI_Mode_Change
	flag,iut_rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Mode_Change",Timeout)
	if(flag~=true) then
		Test_Status = "Fail"
		goto Test_Case_End	
	end
	...
	-- => LT:HCI_Mode_Change
	flag,lt_rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Mode_Change",Timeout)
	if(flag~=true) then
		Test_Status = "Fail"
		goto Test_Case_End
	end

	if(iut_rst_tbl.Current_Mode~=0x00 or iut_rst_tbl.Current_Mode~=0x00 ) then
		Test_Status = "Fail"
		goto Test_Case_End
	end
...
LMP_LIH_BV_16_C("dongle","usb","slave",10)