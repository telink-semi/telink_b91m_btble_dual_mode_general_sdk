...
function LMP_SP_BV_12_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/SP/BV-12-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	flag,lt_rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_User_Passkey_Notification",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_User_Passkey_Request",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_User_Passkey_Request_Reply(iut_handle_w, iut_mode, rst_tbl.BD_ADDR,lt_rst_tbl.PassKey)
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_12_C("dongle","usb","master",10)
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_12_C("dongle","usb","slave",10)
