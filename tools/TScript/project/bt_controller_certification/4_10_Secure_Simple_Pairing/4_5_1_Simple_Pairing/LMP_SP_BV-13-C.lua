...
function LMP_SP_BV_13_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/SP/BV-13-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	IO_Capability = 0x02  -- Keyboard only
	OOB_Data_Present = 0x00 -- No present
	Authentication_Requirement = 0x01 -- MITM Protection Required – Single Profile (0x01)
	...
	-- <= IUT:HCI_IO_Capability_Request_Reply
	IO_Capability = 0x00 -- Display only
	OOB_Data_Present = 0x00 -- No present
	Authentication_Requirement = 0x01  -- MITM Protection Required – Single Profile (0x01)
	...
	flag,iut_rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_User_Passkey_Notification",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_User_Passkey_Request",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_User_Passkey_Request_Reply(lt_handle_w, lt_mode, rst_tbl.BD_ADDR,iut_rst_tbl.PassKey)
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_13_C("dongle","usb","master",10)
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_13_C("dongle","usb","slave",10)
