...
function LMP_SP_BV_21_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/SP/BV-21-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	HCI_Read_Local_OOB_Data(lt_handle_w,lt_mode)
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	lt_oob_commitment 	= array.new(16)
	
	for i=1,16 do
		lt_oob_commitment[i] = rst_tbl.Return_Parameters[i+1]
	end
	
	log_printf(HCI_COMMAND_DISPLAY,"LT OOB COMMITMENT:",lt_oob_commitment,16);

	lt_oob_random 		= array.new(16)

	for i=1,16 do
		lt_oob_random[i] = rst_tbl.Return_Parameters[i+17]
	end

	log_printf(HCI_COMMAND_DISPLAY,"LT OOB RANDOM:",lt_oob_random,16);
	...
	HCI_Authentication_Requested(lt_handle_w,lt_mode,lt_Connection_Handle)
	-- => IUT:HCI Command Status event
	...
	-- => IUT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Link_Key_Request",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_Link_Key_Request_Negative_Reply(lt_handle_w,lt_mode,rst_tbl.BD_ADDR)
	...
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_IO_Capability_Response",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	flag,iut_rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Remote_OOB_Data_Request",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_Remote_OOB_Data_Request_Reply(iut_handle_w, iut_mode, iut_rst_tbl.BD_ADDR,lt_oob_commitment,lt_oob_random)
	-- => IUT:HCI Command Complete event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	-- <= LT:HCI_Remote_OOB_Data_Request_Reply
	HCI_Remote_OOB_Data_Request_Reply(lt_handle_w, lt_mode, lt_rst_tbl.BD_ADDR,iut_oob_commitment,iut_oob_random)
	-- => IUT:HCI Command Complete event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Authentication_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	-- <= LT:HCI_Set_Connection_Encryption
	Encryption_Enable = 1;
	HCI_Set_Connection_Encryption(lt_handle_w,lt_mode,lt_Connection_Handle,Encryption_Enable)
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Encryption_Change",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Encryption_Change",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_21_C("dongle","usb","master",10)
