...
function LMP_SP_BV_18_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/SP/BV-18-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
...
--[[
	...
	HCI_Write_Simple_Pairing_Debug_Mode(lt_handle_w,lt_mode,sp_debug_mode)
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	flag,lt_rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Remote_OOB_Data_Request",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_Remote_OOB_Data_Request_Reply(iut_handle_w, iut_mode, rst_tbl.BD_ADDR,lt_oob_commitment,lt_oob_random)
	-- => IUT:HCI Command Complete event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_18_C("dongle","usb","master",10)
