...
function LMP_SP_BV_32_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP/SP/BV-32-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	-- <= IUT:HCI_IO_Capability_Request_Reply
	reason = 0x38 -- host busy Pairing
	HCI_IO_Capability_Request_Negative_Reply(iut_handle_w, iut_mode, rst_tbl.BD_ADDR,reason)
	-- => IUT:HCI Command Complete event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Command_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_32_C("dongle","usb","master",10)
...
for i=1,Total_times do
	...
	flag = LMP_SP_BV_30_C("dongle","usb","slave",10)
