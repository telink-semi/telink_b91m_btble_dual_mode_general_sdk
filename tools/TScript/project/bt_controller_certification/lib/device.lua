...
function Open_Device(iut_mode,lt_mode)
	...
	elseif(iut_mode=="usb") then
				-- csr dongle as Lower Tester ---
		iut_handle = tl_btusb_init();
		iut_handle_r = iut_handle
		iut_handle_w = iut_handle
		--[[
			----------    Function Map   -----------
			init:	tl_btusb_init();
		...
		if(iut_handle==0) then
			tl_btusb_close(iut_handle)
			log_printf(1,"NOT Find CSR",0,0)
			iut_ret = 1
		else
			log_printf(1,"Open iut device:CSR",0,0)
		end
...
function Close_Device(iut_mode,lt_mode)
	...
	if(iut_mode=="uart" or lt_mode=="uart") then
		tl_rs232_close()
	end
end

-- Bluetooth address of Lower Tester 
-- LT_BD_Addr = {0x13, 0x71, 0xda, 0x7d, 0x1a, 0x00}  --CSR dongle
LT_BD_Addr = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab}  -- eagle ceva ip
-- Bluetooth address of IUT 
-- IUT_BD_Addr= {0x1e, 0xc0, 0xf3, 0x21, 0x32, 0x54}
IUT_BD_Addr= {0x20, 0xcd, 0xc3, 0x67, 0xb5, 0x00}
