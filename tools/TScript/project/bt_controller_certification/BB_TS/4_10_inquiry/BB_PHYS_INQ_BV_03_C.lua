...
function  BB_PHYS_INQ_BV_03_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "BB/PHYS/INQ/BV-03-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	print("============ low tester as slave ,enable sacn:=============")
...
-- <= IUT:HCI Inquiry
	LAP= 0x9e8b33
	Inquiry_Length = 0x30  -- 10*1.28s=12.8s
	Num_Responses  = 2  
...
for i=1,Total_times do
	...
	flag = BB_PHYS_INQ_BV_03_C("dongle","usb","master",70)
	if(flag=="Pass") then
		success=success+1
	else
		fail=fail+1
