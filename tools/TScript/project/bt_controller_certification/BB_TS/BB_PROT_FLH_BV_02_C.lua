...
function  BB_PROT_FLH_BV_02_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "BB/PROT/FLH/BV-01-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	print("============ HCI Change Connection Packet Type:=============")
	Packet_Type = 0x0004
 	HCI_Change_Connection_Packet_Type(lt_handle_w,lt_mode,lt_Connection_Handle,Packet_Type)
	...
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Connection_Packet_Type_Changed",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
    ...
    print("============ ACL data start send round..0..:=============")
	Pb_Flag =0x10
	...
	HCI_Acl_data_send(lt_handle_w,lt_mode,lt_Connection_Handle,Pb_Flag,8,data)
	-- => lt:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Num_Of_Commplete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
	...
	HCI_Acl_data_send(lt_handle_w,lt_mode,lt_Connection_Handle,Pb_Flag,num,data)
	-- => lt:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Num_Of_Commplete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
...
for i=1,Total_times do
	...
	flag = BB_PROT_FLH_BV_02_C("dongle","usb","slave",10)
	if(flag=="Pass") then
		success=success+1
	else
		fail=fail+1
