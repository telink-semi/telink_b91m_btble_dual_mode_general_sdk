...
function LMP_INF_BV_16_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP_INF_BV-16-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	-- <= IUT:HCI_Read_Remote_Ext_Supported_Features
	page_num = 1
	HCI_Read_Remote_Ext_Supported_Features(iut_handle_w,iut_mode,iut_Connection_Handle,page_num)
	...
	-- => IUT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Read_Remote_Ext_Supported_Features_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
...
for i=1,Total_times do
	...
	flag = LMP_INF_BV_16_C("dongle","usb","master",10)
