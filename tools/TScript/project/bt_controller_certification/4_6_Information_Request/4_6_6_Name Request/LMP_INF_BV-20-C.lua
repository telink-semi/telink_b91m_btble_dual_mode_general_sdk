...
function LMP_INF_BV_20_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP_INF_BV-20-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	flag = Simple_paring_Setting(iut_handle_r,iut_handle_w,lt_handle_r,lt_handle_w,iut_mode,lt_mode,role,Timeout)
    ...
    print("============ Lower Tester's LMP SSP (Host) and Controller feature bits are not set. =============")
	print("============Lower Tester's LMP extended feature bit is set. =============")

	HCI_Write_Simple_Pairing_Mode(lt_handle_w,lt_mode,1)
	...
	HCI_Write_Simple_Pairing_Mode(iut_handle_w,iut_mode,1)
...
-- <= IUT:HCI_Set_Event_Mask
	...
	-- => IUT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(iut_handle_r,iut_mode,"HCI_Remote_Name_Request_Ext_feature_notification_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
...
for i=1,Total_times do
	...
	flag = LMP_INF_BV_20_C("dongle","usb","master",10)
