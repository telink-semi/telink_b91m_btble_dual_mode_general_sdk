...
function LMP_INF_BV_21_C(iut_mode,lt_mode,role,Timeout)

	Testcase = "LMP_INF_BV-21-C"
	-- iut_mode = "dongle"
	-- lt_mode  = "usb"
	-- role     = "slave"
	OPTIONAL = ALT1
	...
	print("============ Lower Tester's LMP extended features bit is set. =============")
	print("============ Lower Tester's LMP SSP (Host) Feature bit is set. =============")

	-- <= IUT:HCI_Write_Local_Name
	Local_Name = " IMPLEMENTATION_UNDER_TEST0"
	...
	-- <= LT:HCI_Read_Remote_Ext_Supported_Features
	page_num = 1
	HCI_Read_Remote_Ext_Supported_Features(lt_handle_w,lt_mode,lt_Connection_Handle,page_num)
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
	...
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Read_Remote_Ext_Supported_Features_Complete",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
		goto Test_Case_End
	end
    ...
    Clock_Offset = 0;
	-- <= LT:HCI_Read_Remote_Supported_Features
	HCI_Remote_Name_Request(lt_handle_w,lt_mode,IUT_BD_Addr,Page_Scan_Repetition_Mode,Clock_Offset)
	-- => LT:HCI Command Status event
	flag,rst_tbl = HCI_Wait_Event_Done(lt_handle_r,lt_mode,"HCI_Command_Status",Timeout)
	if(flag~=true) then
		Test_Status="Fail"
...
for i=1,Total_times do
	...
	flag = LMP_INF_BV_21_C("dongle","usb","slave",10)
