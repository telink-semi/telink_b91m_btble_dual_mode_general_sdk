...
HCI_Create_Connection(handle_tb,"usb",addr,0x0008,0x02,
0x00,0x00)
		...
		if(name == "HCI_Connection_Complete") then
			...
			if(flag) then
				if(rst_tbl.Status==0) then
				   print("acl create ok!");
				   CSR_acl_handle = rst_tbl.Connection_Handle;
		...
		if(name2 == "HCI_Connection_Complete") then
			 ...
			 if(flag2) then
				if(rst_tbl2.Status==0) then
				   ...
				   CARA_acl_handle = rst_tbl2.Connection_Handle;
...
HCI_Read_Remote_Supported_Features(handle_tb,"usb",CSR_acl_handle)
step = 1
while(1)
do
    result_tbl2,result_len2 = HCI_result_read(handle_r,"uart");
	...
	if(step == 1) then
		...
		if(flag)then 
			if(name == "HCI_Read_Remote_Supported_Features_Complete")then
				print("HCI_Read_Remote_Supported_Features_Complete");
				HCI_Change_Connection_Packet_Type(handle_tb,"usb",CSR_acl_handle,Packet_Type)   ----Packet Type : 0x0408
				step = 2
			end 
		end
	...
	if(step == 2) then
		...
		if (name == "HCI_Connection_Packet_Type_Changed") then
			if(flag) then
...
---handle
acl_buffer[1] = bit32.band(CSR_acl_handle, 0xff);
acl_buffer[2] = bit32.band(bit32.rshift(CSR_acl_handle,8) , 0x0f);
acl_buffer[2] = bit32.bor(acl_buffer[2],PB_Flag);
acl_buffer[2] = bit32.bor(acl_buffer[2],BC_Flag);
--- data len
acl_buffer[3] = 10;
