...
do 
	...
	if( step == 4) then
		if(name2 == "HCI_Link_Key_Request") then
			...
			print("-------- step 5 ----------");
				...
				HCI_Link_Key_Request_Reply(handle_w,"uart",rst_tbl2.BD_ADDR,LINK_KEY);
				caracara_done = 1;
		...
		if(name == "HCI_Link_Key_Request") then
			...
			--print("-------- step 5 ----------");
				--HCI_Link_Key_Request_Negative_Reply(handle_w,"uart",rst_tbl2.BD_ADDR);
				HCI_Link_Key_Request_Reply(handle_tb,"usb",rst_tbl.BD_ADDR,LINK_KEY);
				csr_done = 1;
				--step = 5; -- to be continue...
			end
		end
	...
	if(step == 5) then
			if(name2 == "HCI_Command_Complete") then
				if(flag2) then
					  if( rst_tbl2.Command_Opcode == 0x040b) then
						--step = 6;
						table.print(rst_tbl2);
						caracara_done = 1;
						print("Caracara hci command done!!");
					  end
			...
			if(name == "HCI_Command_Complete") then
				if(flag) then
					  if( rst_tbl.Command_Opcode == 0x040b) then
						--step = 6;
						table.print(rst_tbl);
						--csr_done = 1;
						print("CSR hci command done!!");
					  end
			...
			if(name == "HCI_Authentication_Complete") then
				if(flag) then
					  if( rst_tbl.Status == 0x00) then
						--step = 6;
						table.print(rst_tbl);
						csr_done = 1;
						print("CSR HCI_Authentication_Complete");
					  end
					--print(string.format("step 5: 0x%04x",rst_tbl.Command_Opcode));
				end
			end
