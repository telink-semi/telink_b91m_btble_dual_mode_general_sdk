...
#if (TLK_MDI_AUDSNK_ENABLE && TLK_ALG_AAC_ENABLE)
#include "drivers.h"
...
#endif
#define TLKMDI_SNK_AAC_DEC_TEMP_SIZE       (4096*2)
#define TLKMDI_SNK_SBC_DEC_TEMP_SIZE       4096

#define TLKMDI_SNK_AAC_ENABLE		  (1 && TLK_ALG_AAC_ENABLE)
#if TLKMDI_SNK_AAC_ENABLE
#define TLKMDI_SNK_AAC_DEC_BUFF_SIZE       27648	//27088
#define TLKMDI_SNK_AAC_SCR_BUFF_SIZE       4096		//4096
#define TLKMDI_SNK_AAC_RAW_RCV_BUFF_SIZE   (1024 * 12)
#define TLKMDI_SNK_AAC_PACKET_FLAG      0x47
#endif

//Total buffer = TEMP(4096)+RECV(10560)+DEC(1248)
//             = 4096+10560+1248 = 15904 + 3*16 = 15952(SBC)
//         AAC =  51K
...
#if TLKMDI_SNK_AAC_ENABLE
#define A2DP_AAC_DEFAULT_BITRATE 	127000
#define AAC_BUFF_NUM_MAX  			18
#define AAC_BUFF_SIZE     			400
#define AUDIO_SAMPLE_RATE_16K  		16000
#define AUDIO_SAMPLE_RATE_441K 		44100
#define AUDIO_SAMPLE_RATE_48K  		48000

uint32 g_aac_frame_copy       = 0;
uint16 g_bt_music_buffer_size = TLKMDI_SNK_AAC_RAW_RCV_BUFF_SIZE;
#endif	// TLKMDI_SNK_AAC_ENABLE
...
extern void tlkcfg_a2dpSnkAacFeatureEnable(void);
...
#if TLKMDI_SNK_AAC_ENABLE
    uint16 enc_buf_size;    /**< SBC: sbc_framesize or AAC: 1200 */
    uint08 enc_buf_id;       /**< id(avdtp frame number) of most updated frame */
    uint08 enc_buf_rptr;     /**< encoded buf read ptr. */
    uint08 enc_buf_wptr;     /**< encoded buf write ptr. */
    uint08 enc_buf_num;      /**< encoded buf num. SBC: 86, AAC: 18 */
    uint16 enc_buf_samples; /**< Samples included in a frame. SBC: 128, AAC: 1024 */
    uint32 enc_para;        /**< AAC codec param. */

    uint08 sync_buf_num;
    uint08 sync_countdown;

    uint08 enc_buf_mute_num;
    uint08 enc_buf_wptr_last;

    uint16 aac_len[AAC_BUFF_NUM_MAX];
    uint16 aac_offset[AAC_BUFF_NUM_MAX];

    uint16 aac_offset_cur;
    uint08 src_low_latency;

    uint08 skip_frames;
    uint16 skip_samples;

    uint08  sbc_framesize;
#endif
...
#if TLKMDI_SNK_AAC_ENABLE
uint08 *g_bt_music_enc_buf_ptr = nullptr;
static uint08 *spTlkMdiSnkAACDecBuff = nullptr;
static uint08 *spTlkMdiSnkAACScrBuff = nullptr;
#endif
...
int tlkmdi_audsnk_init(void)
{
	...
	tlkcfg_a2dpSnkAacFeatureEnable();
	sTlkMdiSnkCtrl.encParma = 0;
	sTlkMdiSnkCtrl.firstInit = true;
	sTlkMdiSnkCtrl.currCodecType = 5;
	#if (TLKBTP_CFG_A2DPSNK_ENABLE)
...
bool tlkmdi_audsnk_switch(uint16 handle, uint08 status)
{
	...
	bool enable;
	uint sampleRate = 0;
	extern uint08 btp_a2dpsnk_getCurrCodec(uint16 aclHandle);
	if (SEPID_AAC == btp_a2dpsnk_getCurrCodec(handle)) {
		sTlkMdiSnkCtrl.currCodecType = SEPID_AAC;
	} else if (SEPID_SBC == btp_a2dpsnk_getCurrCodec(handle)) {
		sTlkMdiSnkCtrl.currCodecType = SEPID_SBC;
	}
...
//        tlkmdi_audsnk_close(handle);
	...
	if(enable){
		#if (TLKMDI_SNK_AAC_ENABLE)
		sTlkMdiSnkCtrl.enc_buf_rptr = 0;
		sTlkMdiSnkCtrl.enc_buf_wptr = 0;
		#endif
...
#if (TLKBTP_CFG_A2DPSNK_ENABLE)
static bool tlkmdi_snk_sbc_initBuffer(bool enable)
{
    bool isSucc = true;

    if (enable) {
        if (spTlkMdiSnkRawRcvBuff == nullptr) {
            spTlkMdiSnkRawRcvBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_RAW_RCV_BUFF_SIZE);
        }

        if (spTlkMdiSnkSBCDecBuff == NULL) {
            spTlkMdiSnkSBCDecBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_SBC_DEC_BUFF_SIZE);
        }

        if(spTlkMdiSnkTmpBuff == nullptr) {
            spTlkMdiSnkTmpBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_SBC_DEC_TEMP_SIZE);
        }

        if (spTlkMdiSnkRawRcvBuff == NULL || spTlkMdiSnkSBCDecBuff == NULL || 
...
#if TLKMDI_SNK_AAC_ENABLE
static bool tlkmdi_snk_aac_initBuffer(bool enable)
{
    bool isSucc = true;

    if (enable) {
    	/* BT MUSIC encode buffer */
       if (g_bt_music_enc_buf_ptr == NULL) {
			g_bt_music_enc_buf_ptr = (uint08 *)tlkmdi_audmem_calloc(TLKMDI_SNK_AAC_RAW_RCV_BUFF_SIZE);
       } 

        /* PCM buffer */
        if(spTlkMdiSnkTmpBuff == nullptr) {
            spTlkMdiSnkTmpBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_AAC_DEC_TEMP_SIZE);
        }

        /* AAC DEC BUFFER */
       if (spTlkMdiSnkAACDecBuff == NULL) {
           spTlkMdiSnkAACDecBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_AAC_DEC_BUFF_SIZE);
       }

        /* AAC SCRATCH BUFFER */
		if (spTlkMdiSnkAACScrBuff == NULL) {
			spTlkMdiSnkAACScrBuff = (uint08*)tlkmdi_audmem_calloc(TLKMDI_SNK_AAC_SCR_BUFF_SIZE);
		}

        if (g_bt_music_enc_buf_ptr == NULL || spTlkMdiSnkTmpBuff == nullptr 
			|| spTlkMdiSnkAACDecBuff == NULL || spTlkMdiSnkAACScrBuff == NULL) {
            isSucc = false;
            enable = false;
            tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN, "malloc error!!!!!");
        }
    }

    if (!enable) {
		if (g_bt_music_enc_buf_ptr != NULL) {
			tlkmdi_audmem_free(g_bt_music_enc_buf_ptr);
			g_bt_music_enc_buf_ptr = NULL;
		}
		...
		if(spTlkMdiSnkAACDecBuff != NULL) {
			tlkmdi_audmem_free(spTlkMdiSnkAACDecBuff);
			spTlkMdiSnkAACDecBuff = NULL;
		}

       if(spTlkMdiSnkAACScrBuff != NULL) {
			tlkmdi_audmem_free(spTlkMdiSnkAACScrBuff);
			spTlkMdiSnkAACScrBuff = NULL;
		}
    }
	tlkmdi_snk_setBuffer(g_bt_music_enc_buf_ptr, TLKMDI_SNK_AAC_RAW_RCV_BUFF_SIZE);
    aac_set_dec_buffer_scratch(spTlkMdiSnkAACDecBuff, spTlkMdiSnkAACScrBuff);
	tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN,
	 			 "tlkmdi_snk_aac_initBuffer: malloc success, enc temp alg scr %p %p %p %p",
	 			 g_bt_music_enc_buf_ptr, spTlkMdiSnkTmpBuff, spTlkMdiSnkAACDecBuff, spTlkMdiSnkAACScrBuff);
    return isSucc;
}
#endif
...
static bool tlkmdi_snk_initBuffer(bool enable)
{
    ...
    if (SEPID_SBC == sTlkMdiSnkCtrl.currCodecType) {
        isSucc = tlkmdi_snk_sbc_initBuffer(enable);
    } else if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
#if TLKMDI_SNK_AAC_ENABLE
        isSucc = tlkmdi_snk_aac_initBuffer(enable);
#endif
    }
...
static void tlkmdi_snk_spkHandler(void)
{
	...
	while(true){
		length = 0;
		#if (TLK_DEV_CODEC_ENABLE)
		if(tlkdev_codec_getSpkIdleLen() >= spkIdleLen){
...
uint08 tlkmdi_snk_push_frames_to_encoded_buff(uint08 id, uint08 *pData, uint16 len)
{
    uint08 ret = 0;
#if (TLKMDI_SNK_AAC_ENABLE)
    sint16 available_enc_buf;
    uint08 overflow = 0;
    uint08 aac_wptr_last;
    sint16 wptr = 0;


    if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
		available_enc_buf = sTlkMdiSnkCtrl.enc_buf_wptr - sTlkMdiSnkCtrl.enc_buf_rptr;

		if (available_enc_buf < 0) {
			available_enc_buf += sTlkMdiSnkCtrl.enc_buf_num;
		}

		if (available_enc_buf >= sTlkMdiSnkCtrl.enc_buf_num - 1) {
			tlkapi_warn(TLKMDI_AUDSNK_DBG_FLAG,
						 TLKMDI_AUDSNK_DBG_SIGN,
						 "bt_music_buff_overflow, drop %d %d %d %d",available_enc_buf, sTlkMdiSnkCtrl.enc_buf_num,
						 sTlkMdiSnkCtrl.enc_buf_wptr, sTlkMdiSnkCtrl.enc_buf_rptr);
...
//        return 0;
    ...
    if (SEPID_SBC == sTlkMdiSnkCtrl.currCodecType) {
        tlkmdi_snk_addRawFrame(id, pData, len);
    } else if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
#if TLKMDI_SNK_AAC_ENABLE
		if (g_bt_music_enc_buf_ptr == nullptr) return 0;
        if (sTlkMdiSnkCtrl.enc_buf_wptr == sTlkMdiSnkCtrl.enc_buf_rptr) {
            sTlkMdiSnkCtrl.aac_offset_cur = 0;
        }

        if (available_enc_buf > 8 && sTlkMdiSnkCtrl.enc_buf_mute_num == 0 &&
            sTlkMdiSnkCtrl.aac_offset_cur < sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_rptr] &&
            sTlkMdiSnkCtrl.aac_offset_cur + len >= sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_rptr]) {
            overflow = 1;
        } else {
            overflow = 0;
        }

        if (overflow == 0 && sTlkMdiSnkCtrl.aac_offset_cur + len > g_bt_music_buffer_size) {
            sTlkMdiSnkCtrl.aac_offset_cur = 0;

            if (available_enc_buf > 8 && sTlkMdiSnkCtrl.enc_buf_mute_num == 0 &&
                len >= sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_rptr]) {
                overflow = 1;
            }
        }

        if ((len <= sTlkMdiSnkCtrl.enc_buf_size && TLKMDI_SNK_AAC_PACKET_FLAG == pData[0]) && overflow == 0) {
        		sTlkMdiSnkCtrl.aac_len[sTlkMdiSnkCtrl.enc_buf_wptr] = len;
				sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_wptr] = sTlkMdiSnkCtrl.aac_offset_cur;
				tmemcpy(g_bt_music_enc_buf_ptr + sTlkMdiSnkCtrl.aac_offset_cur, pData, len);
				sTlkMdiSnkCtrl.aac_offset_cur += len;
        } else {
				aac_wptr_last = sTlkMdiSnkCtrl.enc_buf_wptr ? sTlkMdiSnkCtrl.enc_buf_wptr - 1 : sTlkMdiSnkCtrl.enc_buf_num - 2;

				sTlkMdiSnkCtrl.aac_len[sTlkMdiSnkCtrl.enc_buf_wptr] = sTlkMdiSnkCtrl.aac_len[aac_wptr_last];

				sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_wptr] = sTlkMdiSnkCtrl.aac_offset[aac_wptr_last];

				tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
							 TLKMDI_AUDSNK_DBG_SIGN,
							 "# aac_overflow, duplicate, %d %d %d %d %d %d %d",
							 pData[0], len,
							 sTlkMdiSnkCtrl.enc_buf_id,
							 sTlkMdiSnkCtrl.aac_len[aac_wptr_last],
							 sTlkMdiSnkCtrl.aac_offset[aac_wptr_last],
							 sTlkMdiSnkCtrl.aac_len[sTlkMdiSnkCtrl.enc_buf_rptr],
							 sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_rptr]);
        }

        wptr = sTlkMdiSnkCtrl.enc_buf_wptr + 1;

        if (wptr >= sTlkMdiSnkCtrl.enc_buf_num) {
            sTlkMdiSnkCtrl.enc_buf_wptr = wptr - sTlkMdiSnkCtrl.enc_buf_num;
        } else if (wptr < 0) {
            sTlkMdiSnkCtrl.enc_buf_wptr = wptr + sTlkMdiSnkCtrl.enc_buf_num;
        } else {
            sTlkMdiSnkCtrl.enc_buf_wptr = wptr;
        }
    //    tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN,
	// 				 "sTlkMdiSnkCtrl.enc_buf_wptr %d rptr %d",sTlkMdiSnkCtrl.enc_buf_wptr, sTlkMdiSnkCtrl.enc_buf_rptr);
        return 0;
#endif
    }
...
void tlkmdi_snk_addEncFrame(uint16 aclHandle, uint08 *pData, uint16 dataLen)
{
    ...
    if(sTlkMdiSnkCtrl.handle != aclHandle || sTlkMdiSnkCtrl.enable == 0) {
        return;
    }
...
#if (TLKMDI_SNK_AAC_ENABLE)
        if (pData[16] != 0xB0) {
            wptr     = sTlkMdiSnkCtrl.enc_buf_wptr ? sTlkMdiSnkCtrl.enc_buf_wptr - 1 : sTlkMdiSnkCtrl.enc_buf_num - 2;
            p_last   = g_bt_music_enc_buf_ptr + sTlkMdiSnkCtrl.aac_offset[wptr];
            len_last = sTlkMdiSnkCtrl.aac_len[wptr];

            if (dataLen > 12 && pData[11] == 2 && len_last + dataLen - 12 <= sTlkMdiSnkCtrl.enc_buf_size) {
                tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                             TLKMDI_AUDSNK_DBG_SIGN,
                             "<enc: AAC continue packet> %d %d %d %d",
                             len_last,
                             dataLen,
                             pData[3],
                             sTlkMdiSnkCtrl.enc_buf_id);
                memcpy(p_last + len_last, pData + 12, dataLen - 12);
                len_last += dataLen - 12;
                sTlkMdiSnkCtrl.aac_len[wptr] = len_last;
            } else {
                tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                             TLKMDI_AUDSNK_DBG_SIGN,
                             "<enc: not AAC packet> %d %d %d %d",
                             len_last,
                             dataLen,
                             pData[3],
                             sTlkMdiSnkCtrl.enc_buf_id);
            ...
            /* data is AAC packet */
            enc_para = pData[16] | (pData[17] << 8) | (pData[18] << 16);

            if (sTlkMdiSnkCtrl.enc_para != enc_para) {

                tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                             TLKMDI_AUDSNK_DBG_SIGN,
                             "bt_music_init_aac_decoder_params %d %d",
                             sTlkMdiSnkCtrl.enc_para,
                             enc_para);
                sTlkMdiSnkCtrl.enc_para = enc_para;
//                tlkmdi_snk_init_aac_decoder_params(enc_para, pData, dataLen);
            }

        }
#endif
    } else if (SEPID_SBC == sTlkMdiSnkCtrl.currCodecType) {
        /* 60 02 04 01 80 60 01 41  02 36 ee c5 00 00 00 00
         * 05 9c bd 35 b5 00 95 32  21 00 95
         * SBC packet: L2CAP + AVDTP
         */
        if (pData[13] != 0x9c) {
            tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN, "<enc: not SBC packet, copy last frame>");
            return ;
        }

        enc_para = pData[13] | (pData[14] << 8) | (pData[15] << 16);
        if (sTlkMdiSnkCtrl.encParma != enc_para) {

        	sTlkMdiSnkCtrl.encParma = enc_para;
            tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                         TLKMDI_AUDSNK_DBG_SIGN,
                         "bt_music_init_sbc_decoder_params %d %d %d %d",
                         sTlkMdiSnkCtrl.encParma,
                         enc_para);
        }
    }
    
//************************************************************************************
    if (sTlkMdiSnkCtrl.firstInit || sTlkMdiSnkCtrl.encParma != enc_para) {
        uint32 sampleRate;
        sampleRate = sTlkMdiSnkCtrl.sampleRate;

        if (SEPID_SBC == sTlkMdiSnkCtrl.currCodecType) {
            tlkapi_qfifo_clear(&spTlkMdiSnkFifo);
            tlkmdi_snk_firstInit(enc_para, pData, dataLen);
        } else if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
#if TLKMDI_SNK_AAC_ENABLE
            tlkmdi_snk_init_aac_decoder_params(enc_para, pData, dataLen);
#endif
...
#if (TLKMDI_AUDSNK_DOUBLE_CHANNEL_ENABLE)
            sTlkMdiSnkCtrl.decFunc = (tlkmdi_snk_decFunc)tlkalg_aac_dec_stereo;
#else
            sTlkMdiSnkCtrl.decFunc = (tlkmdi_snk_decFunc)tlkalg_aac_dec_chn_left;
#endif
        }
    }
    ...
    if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
#if TLKMDI_SNK_AAC_ENABLE
        /* Media header is 12 bytes */
        aac_pkt_len = dataLen - 12;

        if (TLKMDI_SNK_AAC_PACKET_FLAG != pData[12] || aac_pkt_len > sTlkMdiSnkCtrl.enc_buf_size - 2) {
            tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                         TLKMDI_AUDSNK_DBG_SIGN,
                         "# aac enc rec err, aac_pkt_len: %d, enc_buf_size: %d, pData[12]: %d",
                         aac_pkt_len, sTlkMdiSnkCtrl.enc_buf_size, pData[12]);
        } else {
            tlkmdi_snk_push_frames_to_encoded_buff(pData[3], pData + 12, aac_pkt_len);
            if(sTlkMdiSnkCtrl.waitFrame != 0) {
				sTlkMdiSnkCtrl.waitFrame--;
			}
        }

#endif
    } else if (SEPID_SBC == sTlkMdiSnkCtrl.currCodecType) {
        if (pData[12] * sTlkMdiSnkCtrl.frameSize == dataLen - 13) {
            tlkmdi_snk_push_frames_to_encoded_buff(pData[3], pData + 13, dataLen - 13);
            if(sTlkMdiSnkCtrl.waitFrame != 0) {
                sTlkMdiSnkCtrl.waitFrame--;
            }
        } else {
            tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                         TLKMDI_AUDSNK_DBG_SIGN,
                         "<enc: receive packet length error> %d %d",
                         pData[12],
                         dataLen - 13);
        }
    }
...
static uint tlkmdi_snk_getPlaybackData(uint08 *pBuffer)
{
    ...
    if (SEPID_AAC == sTlkMdiSnkCtrl.currCodecType) {
        ...
        if (sTlkMdiSnkCtrl.enc_buf_rptr != sTlkMdiSnkCtrl.enc_buf_wptr) {

            buf_ptr = g_bt_music_enc_buf_ptr + sTlkMdiSnkCtrl.aac_offset[sTlkMdiSnkCtrl.enc_buf_rptr];

            if (NULL == buf_ptr || TLKMDI_SNK_AAC_PACKET_FLAG != buf_ptr[0]) {
				tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN, "# AAC enc pkt out err ", buf_ptr);
				outLen = 0;
				ret = 0xfe;
			} else {
				enc_data_len = sTlkMdiSnkCtrl.aac_len[sTlkMdiSnkCtrl.enc_buf_rptr];

				uint08* pdes = pBuffer;
				ret = sTlkMdiSnkCtrl.decFunc(buf_ptr, enc_data_len, pdes);

				outLen = 4096;
				sTlkMdiSnkCtrl.enc_buf_rptr += 1;
				if (sTlkMdiSnkCtrl.enc_buf_rptr >= sTlkMdiSnkCtrl.enc_buf_num) {
					sTlkMdiSnkCtrl.enc_buf_rptr -= sTlkMdiSnkCtrl.enc_buf_num;
				}

//				log_b8(SL_STACK_SCO_EN, SL08_bt_enc_buff_rptr, sTlkMdiSnkCtrl.enc_buf_rptr);
        	...
        	outLen = 4096;
        	// tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG, TLKMDI_AUDSNK_DBG_SIGN, "fill mute data rptr %d, wptr %d",
        	// 				sTlkMdiSnkCtrl.enc_buf_rptr, sTlkMdiSnkCtrl.enc_buf_wptr);
        }
...
#if TLKMDI_SNK_AAC_ENABLE
uint16 tlkmdi_snk_aac_set_buffer(uint08 *p_buf)
{
    g_bt_music_enc_buf_ptr = p_buf;

    return TLKMDI_SNK_AAC_RAW_RCV_BUFF_SIZE;
}

uint08 *tlkmdi_snk_aac_get_buffer(void)
{
    return g_bt_music_enc_buf_ptr;
}

void tlkmdi_snk_aac_set_param(uint08 num_of_buff, uint16 size, uint16 samples)
{
    sTlkMdiSnkCtrl.enc_buf_num     = num_of_buff;
    sTlkMdiSnkCtrl.enc_buf_size    = size;
    sTlkMdiSnkCtrl.enc_buf_samples = samples;
}

void tlkmdi_snk_init_aac_decoder_params(uint32 param, uint08 *pData, uint16 len)
{
    uint16 sample_rate = 0;
    uint08 buffer_num;

    sample_rate = (0x8C == pData[17]) ? AUDIO_SAMPLE_RATE_48K : AUDIO_SAMPLE_RATE_441K;

    buffer_num = g_bt_music_buffer_size / AAC_BUFF_SIZE;
    if (buffer_num > AAC_BUFF_NUM_MAX) {
        buffer_num = AAC_BUFF_NUM_MAX;
    }

    tlkmdi_snk_aac_set_param(buffer_num, 1200, 1024);

    sTlkMdiSnkCtrl.enc_para = param;
    sTlkMdiSnkCtrl.sampleRate = sample_rate;
    ...
    tlkapi_trace(TLKMDI_AUDSNK_DBG_FLAG,
                 TLKMDI_AUDSNK_DBG_SIGN,
                 "<aac enc: set sample rate> %d %d %d %d",
                 sTlkMdiSnkCtrl.enc_buf_samples,
                 sample_rate,
                 sTlkMdiSnkCtrl.enc_buf_size,
                 sTlkMdiSnkCtrl.enc_buf_num);
}
