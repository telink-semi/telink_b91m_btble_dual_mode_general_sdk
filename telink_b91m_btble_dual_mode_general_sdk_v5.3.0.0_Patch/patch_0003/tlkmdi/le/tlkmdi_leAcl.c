...
//	s8 rssi = pa->data[pa->len];/
	...
	#if (TLKMDI_LE_CENTRAL_SMP_ENABLE)
	u32	central_auto_connect = blc_smp_searchBondingSlaveDevice_by_PeerMacAddress(pa->adr_type, pa->mac);
	#else
	u32	central_auto_connect = 0;
	#endif

#if 0 //This function is only used for debugging.
	if(pa->event_type == ADV_REPORT_EVENT_TYPE_SCAN_RSP){
	  tlkapi_trace(TLKMDI_LE_COMMAND_FLAG, TLKMDI_LE_COMMAND_SIGN, "ADV_REPORT_EVENT_TYPE_SCAN_RSP mac:%02x %02x %02x %02x %02x %02x",\
