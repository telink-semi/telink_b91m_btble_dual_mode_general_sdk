...
* BLE
   - Fixed the issue: when MCU parses the create connection command from the PC tool, it uses an incorrect offset to get the MAC address.
...
* BLE
   - 修复issue：当MCU从PC工具解析创建连接命令时，它使用了错误的偏移量来获取MAC地址。
